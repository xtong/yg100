...
class GuardianTestCase(TestCase):
    ...
    def test_add_new_students(self):
        ...
        # 1 is reserved for sentinel student when initializing
        ...
        for query in queryset:
            ...
            guardianship_data = {'parent_id': query['parent_id'],
                                'student_id': '1',
                                'relationship': query['relationship'],}
            for student_id in student_id_list:
                guardianship_data['student_id'] = student_id['student_id']
                response = client.post(
                    reverse('guardianship-list'),
                    guardianship_data,
                    format='json'
    ...
    def test_add_new_parent(self):

        client = APIClient()

        parent_id_list = list()
        # 1 is reserved for sentinel student when initializing a study class
        parent_id_list.append({'parent_id': '3', 'relationship': Guardianship.GRANDPA})
        parent_id_list.append({'parent_id': '4', 'relationship': Guardianship.GRANDPA})
        ...
        for query in queryset:
            ...
            guardianship_data = {'parent_id': '1',
                            'student_id': query['student_id'],
                            'relationship': query['relationship'],
                            }
            for parent_id in parent_id_list:
                guardianship_data['parent_id'] = parent_id['parent_id']
                response = client.post(
                    reverse('guardianship-list'),
                    guardianship_data,
                    format='json'
